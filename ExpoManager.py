...
os.environ["OPENAI_API_KEY"]="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"
...
with open("C:/Users/Alex/Desktop/AIOutput.txt", "w") as f:
...
while(True):
  ...
  for j in char_list:
     if(i == 'input Talk to '+j.name+''):
        ...
        with open("C:/Users/Alex/OneDrive/AIOutput.txt", "w") as f:
          response = f.write(story)
        action("SetDialog(\""+story3+" [Xbox|Close Dialog]\" )")
        action("SetRight(\""+j.name+"\")")
        action("ShowDialog()")
        if(i == 'input Selected xbox'):
           action("HideDialog()")
           action('EnableInput()')
           action("EnableInput()")
        #time.sleep(15)
  ...
  if(i == 'input Review Conversations BlueBook'):
     ...
     action("SetNarration(This works)")
